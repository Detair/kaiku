...
//!
//! - `core` — guild CRUD, members, settings, channel listing/reorder, bots
//!   and slash commands (used by `handlers.rs`)
...
//! - `limits` — reusable count queries shared by enforcement checks and the
//!   usage stats endpoint (used by `handlers.rs`, formerly `guild/limits.rs`)
//!
