...
async fn test_mention_everyone_blocked_without_permission(pool: PgPool) -> sqlx::Result<()> {
    // Create guild owner
    let owner = db::create_user(&pool, "owner", "Owner", Some("owner@example.com"), "password123").await?;

    // Create regular test user (not owner)
    let user = db::create_user(&pool, "testuser", "Test User", Some("testuser@example.com"), "password123").await?;
    ...
    sqlx::query(
        "INSERT INTO channels (id, name, channel_type, guild_id) VALUES ($1, $2, $3, $4)"
    )
    .bind(channel_id)
    .bind("test-channel")
    ...
    assert!(!ctx.has_permission(GuildPermissions::MENTION_EVERYONE),
...
async fn test_mention_everyone_allowed_with_permission(pool: PgPool) -> sqlx::Result<()> {
    // Create test user
    let user = db::create_user(&pool, "moderator", "Moderator", Some("mod@example.com"), "password123").await?;
...
async fn test_guild_owner_can_mention_everyone(pool: PgPool) -> sqlx::Result<()> {
    // Create test user
    let owner = db::create_user(&pool, "owner", "Guild Owner", Some("owner@example.com"), "password123").await?;
    ...
    assert_eq!(ctx.computed_permissions, GuildPermissions::all(),
