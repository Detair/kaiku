...
//!
//! Run with: `cargo test --test filters_http_test -- --nocapture`
...
async fn setup_guild_with_filters(app: &TestApp) -> (Uuid, Uuid, Uuid, String) {
    ...
    let channel_id = helpers::create_channel(&app.pool, guild_id, "filter-test").await;
...
async fn test_dm_message_not_filtered() {
    ...
    let dm_channel = helpers::create_dm_channel(&app.pool, user_a, user_b).await;

    let mut guard = app.cleanup_guard();
    guard.add(move |pool| async move { helpers::delete_dm_channel(&pool, dm_channel).await });
